...
def train_actor_and_critic(optimizer, memory, q_val, gamma, entropy_coeff, reward_fn):
    """
    ...
    for i, (_, _, _, _, reward, done, _, obs) in enumerate(memory.reversed()):
        q_val = reward_fn(obs) + gamma * q_val * (1.0 - done)
...
def create_a2c_agent(env,
    ...
    n_step_update = lt.liveVar(hyper_params["n_step_update"], "n_step_update")
...
def main():
    ...
    for _ in range(3):
        ...
        actor, _, _ = create_a2c_agent(env=env,
                                       ...
                                       user_reward_params=sparse_reward_fn,
                                       plot_results=True)
