...
class DDQN_Agent:
    ...
    def train(self, batch_size, reward_fn, loss_fn=torch.nn.MSELoss()):
        """
        ...
        states, actions, rewards, next_states, obss, dones = \
            self.experience_replay.sample_from_experience(sample_size=batch_size)
        rewards = map(reward_fn, obss)
...
def create_ddqn_agent(env,
    ...
    :param env:
    :param hyper_params:
    :param user_reward_params: tuple, the user-defined reward function parameters
    ...
    learning_rate = lt.liveVar(hyper_params["lr"], "lr")
    update_steps = lt.liveVar(hyper_params["update_steps"], "update_steps")
    epsilon = lt.liveVar(0.5, "epsilon")
    ...
    try:
        ...
        for episode_idx in tqdm(range(num_episodes)):
            ...
            if reward_scaling_factor.changed() or reward_for_h_and_t.changed() or reward_for_h_and_nt.changed() or reward_for_nh_and_t.changed() or reward_for_nh_and_nt.changed():
                user_reward_fn = get_reward_fn(reward_scaling_factor, reward_for_h_and_t, reward_for_h_and_nt, reward_for_nh_and_t, reward_for_nh_and_nt)
            if learning_rate.changed():
                agent.optimizer = torch.optim.Adam(agent.q_net.parameters(), lr=learning_rate())
            if gamma.changed():
                agent.gamma = torch.tensor(gamma()).float()
            while not done:
                ep_len += 1
                ...
                if user_reward_fn is not None:
                    reward = user_reward_fn(state=obs)
                ...
                if total_steps % update_steps() == 0 and len(agent.experience_replay) > batch_size:
                    for _ in range(4):  # for 75000 steps, 4 * 60 seems to work better
                        num_updates += 1
                        loss = agent.train(batch_size=batch_size, reward_fn=user_reward_fn)
                        losses += loss
