...
def get_reward_fn(reward_scaling_factor, reward_for_h_and_t, reward_for_h_and_nt, reward_for_nh_and_t, reward_for_nh_and_nt):
    def compute_reward(state):
        """
        Using the user-provided weights, compute the reward function
        """
        if state["hungry"] and state["thirsty"]:
            return reward_scaling_factor() * reward_for_h_and_t()
        elif state["hungry"] and not state["thirsty"]:
            return reward_scaling_factor() * reward_for_h_and_nt()
        elif not state["hungry"] and state["thirsty"]:
            return reward_scaling_factor() * reward_for_nh_and_t()
        elif not state["hungry"] and not state["thirsty"]:
            return reward_scaling_factor() * reward_for_nh_and_nt()
...
class PPO:
    ...
    def update(self, c1=0.5, entropy_coeff=0.01, reward_fn=None):
        """
        ...
        rewards = []
        discounted_reward = 0
        for obs, is_terminal in zip(reversed(self.rollout_buffer.obs),
                                       reversed(self.rollout_buffer.is_terminals)):
            if is_terminal:
                discounted_reward = 0
            discounted_reward = reward_fn(obs) + (self.gamma * discounted_reward)
...
def train_ppo_agent(env,
    ...
    gamma = lt.liveVar(hyper_params["gamma"], "gamma")
    lr = lt.liveVar(hyper_params["lr"], "lr")
    update_steps = lt.liveVar(hyper_params["update_steps"], "update_steps")
    eps_clip = lt.liveVar(hyper_params["eps_clip"], "eps_clip")
    entropy_coeff = lt.liveVar(hyper_params["entropy_coeff"], "entropy_coeff")

    reward_scaling_factor, reward_for_h_and_t, reward_for_h_and_nt, reward_for_nh_and_t, reward_for_nh_and_nt = user_reward_params
    reward_scaling_factor : lt.liveVar
    reward_for_h_and_t : lt.liveVar
    reward_for_h_and_nt : lt.liveVar
    reward_for_nh_and_t : lt.liveVar
    reward_for_nh_and_nt : lt.liveVar
    user_reward_fn = get_reward_fn(reward_scaling_factor, reward_for_h_and_t, reward_for_h_and_nt, reward_for_nh_and_t, reward_for_nh_and_nt)
    ...
    try:
        for episode_idx in tqdm(range(num_episodes)):
            ...
            if reward_scaling_factor.changed() or reward_for_h_and_t.changed() or reward_for_h_and_nt.changed() or reward_for_nh_and_t.changed() or reward_for_nh_and_nt.changed():
                user_reward_fn = get_reward_fn(reward_scaling_factor, reward_for_h_and_t, reward_for_h_and_nt, reward_for_nh_and_t, reward_for_nh_and_nt)
            if lr.changed():
                ppo_agent.optimizer = torch.optim.Adam(ppo_agent.q_net.parameters(), lr=lr())
            if gamma.changed():
                ppo_agent.gamma = torch.tensor(gamma()).float()
            if eps_clip.changed():
                ppo_agent.eps_clip = torch.tensor(eps_clip()).float()
            while not done:
                ...
                if user_reward_fn is not None:
                    reward = user_reward_fn(state=state)
                ...
                if time_step % update_steps() == 0:
                    num_updates += 1
                    ppo_agent.update(entropy_coeff=entropy_coeff(), reward_fn=user_reward_fn)
...
def create_ppo_agent(env, hyper_params, user_reward_params, plot_state_visit_distribution=False, plot_results=True):
    """
    ...
    ppo_agent, reward_list, fitness_list = train_ppo_agent(env=env,
                                                           ...
                                                           user_reward_params=user_reward_params,
