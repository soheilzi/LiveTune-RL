...
class InteractiveLearningCurvePlot:
    ...
    def __init__(self, num_axes):
        ...
        plt.ioff()
        plt.ion()
        self.fig, self.axes = plt.subplots(num_axes, figsize=(12, 8))
        self.lines = []
        self.trend_lines = []
...
class ExperienceReplay:
    ...
    def collect_experience(self, state, action, reward, next_state, obs, done):
        """
        ...
        :return: None
        """
        self.experience.append([state, action, reward, next_state, obs, done])
    ...
    def sample_from_experience(self, sample_size):
        ...
        obs = [exp[4] for exp in sample]
        dones = torch.tensor([exp[5] for exp in sample]).float()
        return s, a, rn, sn, obs, dones
...
class RolloutBuffer:
    ...
    def add_experience(self, action, state, logprob, entropy, reward, obs, done, value_estimate=None):
        """
